...
import type { TenantContextValue } from '../../app/tenant';
...
export const Topbar = ({
  ...
  <header className="flex h-16 items-center gap-3 border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-raised)] px-4">
    ...
    <label className="hidden text-xs font-semibold text-[var(--color-ink-muted)] sm:flex sm:items-center sm:gap-1.5">
      Obra:
      <select
        defaultValue={tenant.activeWorksiteId}
    ...
    <div className="flex shrink-0 items-center gap-2 pl-1">
      <span className="grid size-8 place-items-center rounded-full bg-[var(--color-brand-100)] text-xs font-black text-[var(--color-brand-700)]">
        {tenant.userName.charAt(0)}
      </span>
      <span className="hidden text-left leading-tight lg:block">
        <span className="block text-xs font-bold text-[var(--color-ink-primary)]">{tenant.userName}</span>
        <span className="block text-[10px] font-semibold uppercase tracking-wide text-[var(--color-ink-muted)]">{tenant.userRole}</span>
      </span>
    </div>
  </header>
);
