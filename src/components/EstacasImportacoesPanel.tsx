...
import { buildStakeImportApplication } from '../imports/stakeImportApplication';
import type { ControleEstacas, CravacaoEstaca, LoteEstaca } from '../types';

interface Props {
  controle: ControleEstacas;
  responsavel: string;
  onApply: (lotes: LoteEstaca[], cravacoes: CravacaoEstaca[]) => void;
...
export default function EstacasImportacoesPanel({ controle, responsavel, onApply, onError }: Props) {
  ...
  const handleFileSelected = async (file: File) => {
    ...
    try {
      ...
      const result = runImportPipeline(workbook, domain => (domain === 'stakes' ? controle : undefined));
  ...
  return <div className="mt-3 space-y-3">
    ...
    <SpreadsheetImportReview
      ...
      title="Prévia de importação — Estacas"
      fileName={fileName}
      ...
      onConfirm={() => {
        ...
        const applied = buildStakeImportApplication(preview, controle, responsavel);
        onApply(applied.lotes, applied.cravacoes);
        // Nenhuma linha pode sumir em silêncio: o dry-run mostra "Novo" com
        // base só na chave operacional do adaptador, mas a aplicação real
        // ainda exige campos completos (NF/descrição/comprimento para lote,
        // identificação/comprimento para cravação) — por isso o resumo
        // depois de aplicar sempre mostra o que foi de fato gravado e o que
        // ficou de fora, mesmo quando os números não batem com a prévia.
        setResultSummary(
          `Aplicado: ${applied.lotes.length} lote(s) e ${applied.cravacoes.length} cravação(ões). `
          + `Fora do lote: ${applied.skipped.duplicate} duplicada(s), ${applied.skipped.review} em conferência (dado incompleto), `
          + `${applied.skipped.reference} de aba só de referência (sem registro próprio)${applied.skipped.other ? `, ${applied.skipped.other} outra(s)` : ''}.`,
        );
        setPreview(null);
        setDryRunRan(false);
        setTruncationNote('');
