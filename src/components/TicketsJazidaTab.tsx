...
const drawTicketPairOnPdf = (doc: jsPDF, releaseTicket: TicketJazida, receiptTicket: TicketJazida, logo: string) => {
  const drawCopy = (ticket: TicketJazida, top: number, copyLabel: string) => {
    ...
    label('Quantidade (m³ / caçamba)', left + 3, detailTop + 4); value(ticket.quantidadeM3 > 0 ? `${ticket.quantidadeM3} ${ticket.unidadeQuantidade || 'm³'}` : '', left + 3, detailTop + 10);
...
export default function TicketsJazidaTab({ tickets, onSaveTicket, onDeleteTicket, onImportTickets, onReserveTicketNumber }: TicketsJazidaTabProps) {
  ...
  const buildBlankBatchTicket = (number: string, type: TipoTicketJazida): TicketJazida => ({
    id: `ticket-impressao-branco-${type === 'Liberação' ? 'lib' : 'rec'}-${number}`,
    data: '',
    tipoTicket: type,
    ticketNumero: number,
    prefixo: '',
    ...
    horaSaida: '',
    horaChegada: '',
    tipoMaterial: '' as TipoMaterialJazida,
    quantidadeM3: 0,
    destinoObra: '' as DestinoObraJazida,
    destinoOutro: '',
    responsavelLiberacao: '',
    nomeLegivel: '',
    empresa: '' as EmpresaTicketJazida,
    estaca: '',
    observacao: '',
    unidadeQuantidade: 'm³',
  });
  ...
  const handleGenerateBatchTickets = async () => {
    setValidationError('');
    setImportMessage('');
    const range = Math.max(0, Math.min(200, Math.floor(Number(batchSequenceRange) || 0)));
    ...
    try {
      const pairs = numbers.map(number => ({
        releaseTicket: buildBlankBatchTicket(number, 'Liberação'),
      ...
      console.error('Erro ao gerar tickets em branco:', err);
      setValidationError('Não foi possível gerar o PDF da sequência de tickets em branco.');
    } finally {
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 max-w-3xl w-full space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1">Gera somente o PDF com tickets em branco. Nenhum ticket é cadastrado no sistema.</p>
              </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950 p-3 text-[10px] text-slate-400">
              Serão impressos {Math.max(0, Math.min(200, Number(batchSequenceRange) || 0)) + 1} números: {batchStartNumber ? formatSequentialNumber(batchStartNumber, 0) : '-'} até {batchStartNumber ? formatSequentialNumber(batchStartNumber, Math.max(0, Math.min(200, Number(batchSequenceRange) || 0))) : '-'}.
            </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={handleGenerateBatchTickets} disabled={isBatchPrinting} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-emerald-500 disabled:opacity-60">
                {isBatchPrinting ? 'Gerando PDF...' : 'Imprimir em branco'}
              </button>
            </div>
          </div>
        </div>
