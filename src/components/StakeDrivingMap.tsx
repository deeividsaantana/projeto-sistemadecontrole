...
import type { CravacaoEstaca, ObraLocal } from '../types';
...
export default function StakeDrivingMap({ items, obras, activeId, onActiveIdChange, onVisibleIdsChange }: Props) {
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    return (!q || [item.identificacao, item.item, item.perfil].some(value => String(value || '').toLocaleLowerCase('pt-BR').includes(q)))
      && (!date || item.data === date) && (!trechoId || item.obraLocalId === trechoId)
      && (!profile || item.perfil === profile) && (!status || statusOf(item).key === status);
  }).sort(sequenceSort), [items, search, date, trechoId, profile, status]);
  ...
  const ramoName = (_id?: string) => 'Não informado';
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
    ...
    <div className="mb-4 grid gap-2 md:grid-cols-3 xl:grid-cols-6"><label className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Pesquisar estaca" className="w-full rounded-xl border border-slate-200 py-2 pl-9 pr-3 text-sm outline-none focus:border-emerald-500"/></label><input type="date" value={date} onChange={event => setDate(event.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700 outline-none focus:border-emerald-500"/><select value={trechoId} onChange={event => setTrechoId(event.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700"><option value="">Todos os trechos/obras</option>{obras.map(item => <option key={item.id} value={item.id}>{item.nome}</option>)}</select><select value={profile} onChange={event => setProfile(event.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700"><option value="">Todos os perfis</option>{profiles.map(item => <option key={item}>{item}</option>)}</select><select value={status} onChange={event => setStatus(event.target.value as StatusKey | '')} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700"><option value="">Todos os status</option><option value="pending">Não iniciada</option><option value="active">Em cravação</option><option value="done">Concluída</option><option value="divergence">Divergência</option></select></div>
