    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
        ...
        <article className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
          <header className="flex items-center justify-between border-b border-slate-800 p-4"><div><h2 className="font-bold text-white">Acessos cadastrados</h2><p className="text-xs text-slate-500">Perfis aplicados também no Firebase Authentication</p></div><button type="button" onClick={() => void refresh()} disabled={loading || saving} className="rounded-md border border-slate-700 p-2 text-slate-300 hover:border-emerald-500"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button></header>
