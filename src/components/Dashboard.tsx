...
import { fleetSnapshot, fleetSummary, localToday } from '../utils/dashboardOperational';
...
const normalizeFleetStatus = (status: string): Exclude<FleetFilter, 'Todos'> | 'À disposição' => {
  ...
  if (['Disponível', 'À disposição', 'Liberado'].includes(status)) return 'À disposição';
  // Status legado ou vazio não pode aparecer como equipamento disponível.
  return 'A confirmar';
};
...
export default function Dashboard({
  ...
  const fleetSeries = useMemo(() => {
    ...
    return dates.map(date => {
      ...
      const summary = fleetSummary(fleetSnapshot(equipamentos, controlesEquipamentos, date));
      return {
        date, records, operating: summary.operating, maintenance: summary.maintenance,
        confirm: summary.pending + summary.unknown, available: summary.available,
        availability: summary.availability ?? 0, coverage: summary.coverage ?? 0, total: summary.total,
        semDados: summary.confirmed === 0,
      };
    });
  }, [controlesEquipamentos, equipamentos, periodDays]);
  ...
  const latest = [...fleetSeries].reverse().find(item => !item.semDados) || fleetSeries.at(-1) || {
    ...
    maintenance: 0, confirm: 0, available: 0, availability: 0, coverage: 0, total: 0,
  };
  ...
  return (
    <main ref={dashboardRef} id="dashboard-tab" className="erp-dashboard min-h-full bg-white pb-14 text-[#172329]">
      ...
      <div className="w-full px-3 pb-8 sm:px-5">
        ...
        <Panel title="Distribuição da frota" className="mt-4" action={<span className="text-[10px] font-bold uppercase tracking-[.13em] text-[#748187]">{latest.date ? 'posição de ' + formatDate(latest.date) : 'sem lançamento'} · {latest.total} equipamento(s)</span>}>
          <StatusDistribution segments={fleetSegments} total={latest.total} active={fleetFilter} onSelect={chooseFilter} />
        </Panel>
