...
import { normalizeEmployeeCode } from '../utils/canonicalIdentity';
...
type ViewMode = 'dashboard' | 'lancamento' | 'registros' | 'deficiencias' | 'indicadores-sge';
...
const ParteDiariaEquipamentosTab: React.FC<ParteDiariaEquipamentosTabProps> = ({ registros, equipamentos, funcionarios, obras, onSave, onDelete, onDeleteMany, onImport }) => {
  ...
  const dataFieldRef = useRef<HTMLInputElement>(null);

  // Foco automático no campo Data ao entrar no lançamento (novo ou edição),
  // e atalhos de teclado: Ctrl+Enter salva, Esc volta para a lista sem salvar.
  useEffect(() => {
    if (view !== 'lancamento') return;
    const raf = requestAnimationFrame(() => dataFieldRef.current?.focus());
  ...
  const activeOperators = useMemo(() => funcionarios.filter(item => item.ativo && !['INATIVO', 'DESMOBILIZADO'].includes(item.status || '')).sort((a, b) => a.nome.localeCompare(b.nome)), [funcionarios]);
  ...
  const updateOperatorByCode = (code: string) => {
    const normalizedCode = normalizeEmployeeCode(code);
    const item = funcionarios.find(employee => normalizeEmployeeCode(employee.matricula) === normalizedCode);
    setForm(current => ({
      ...current,
      matricula: code,
      ...(item ? { operadorId: item.id, operadorNome: item.nome, matricula: item.matricula || code } : {}),
    }));
    if (code.trim() && !item) setError(`Nenhum colaborador ativo localizado com o código/matrícula ${code}. O preenchimento manual continua liberado.`);
    else setError('');
  };
  ...
  const handleLancamentoKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
  ...
  return (
    <div className="space-y-5 text-slate-100">
      ...
      {!['lancamento', 'indicadores-sge'].includes(view) && <section className="grid gap-3 border-b border-slate-800 pb-5 md:grid-cols-2 xl:grid-cols-[1.4fr_repeat(5,1fr)_auto]">
      ...
      {view === 'lancamento' && <div className="space-y-5" onKeyDown={handleLancamentoKeyDown}>
        ...
        <section className="border border-slate-800 bg-slate-950"><div className="flex items-center gap-2 border-b border-slate-800 p-4"><ClipboardCheck size={18} className="text-emerald-300" /><h3 className="font-bold text-white">Identificação da ficha</h3></div><div className="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="text-xs font-bold uppercase text-slate-400">Data<input ref={dataFieldRef} type="date" value={form.data} onChange={event => setForm(current => ({ ...current, data: event.target.value }))} className="mt-2 h-11 w-full border border-slate-700 bg-slate-900 px-3 text-base text-white outline-none focus:border-emerald-500" /></label>
          ...
          <label className="text-xs font-bold uppercase text-slate-400">Código / matrícula<input value={form.matricula} onChange={event => updateOperatorByCode(event.target.value)} placeholder="Digite o código para localizar" className="mt-2 h-11 w-full border border-slate-700 bg-slate-900 px-3 text-base text-white outline-none focus:border-emerald-500" /><span className="mt-1 block text-[10px] normal-case text-slate-500">Ao encontrar o código, nome e cadastro são vinculados automaticamente.</span></label>
        ...
        <div className="sticky bottom-3 z-10 flex flex-col gap-2 border border-slate-700 bg-slate-950/95 p-3 shadow-2xl backdrop-blur md:flex-row md:items-center md:justify-between"><div className="flex flex-wrap gap-4 text-sm"><span>Trabalhado: <strong className="text-emerald-300">{formatNumber(getWorkedHours(form))} h</strong></span><span>Parado: <strong className="text-amber-300">{formatNumber(getStoppedHours(form))} h</strong></span><span>Status: <strong>{deriveStatus(buildFinalRecord())}</strong></span></div><div className="flex items-center gap-3"><span className="hidden text-[10px] font-semibold text-slate-500 md:inline">Ctrl+Enter salva · Esc volta</span><button onClick={() => setView('registros')} className="h-11 border border-slate-700 bg-slate-900 px-4 text-sm font-bold">Cancelar</button><button onClick={saveRecord} className="inline-flex h-11 items-center gap-2 bg-emerald-500 px-5 text-sm font-bold text-slate-950"><Save size={18} /> Salvar parte diária</button></div></div>
      </div>}

      {view === 'registros' && <section className="overflow-hidden border border-slate-800 bg-slate-950"><div className="flex items-center justify-between border-b border-slate-800 p-4"><div><h2 className="font-bold text-white">Partes diárias cadastradas</h2><span className="text-xs text-slate-500">{filtered.length} resultado(s) · {selectedRecordIds.length} selecionada(s)</span></div><div className="flex items-center gap-2"><button type="button" onClick={()=>setSelectedRecordIds(selectedRecordIds.length===filtered.length?[]:filtered.map(item=>item.id))} className="h-9 border border-slate-700 px-3 text-xs font-bold">{selectedRecordIds.length===filtered.length?'Limpar seleção':'Selecionar todas'}</button><button type="button" disabled={!selectedRecordIds.length} onClick={()=>{onDeleteMany(selectedRecordIds);setSelectedRecordIds([])}} className="h-9 bg-rose-600 px-3 text-xs font-bold text-white disabled:opacity-40">Excluir selecionadas</button><ListFilter className="text-slate-400" size={20} /></div></div><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-slate-900 text-xs uppercase text-slate-500"><tr>{['Selecionar', 'Nº / data', 'Equipamento', 'Operador', 'Obra', 'Trabalhado', 'Parado', 'Checklist', 'Status', 'Ações'].map(label => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-800">{filtered.map(item => { const deficiencies = item.checklist.filter(check => check.resposta === 'Não').length + (item.outrosProblemas ? 1 : 0); return <tr key={item.id} className="hover:bg-slate-900/60"><td className="px-3"><input type="checkbox" checked={selectedRecordIds.includes(item.id)} onChange={()=>setSelectedRecordIds(current=>current.includes(item.id)?current.filter(id=>id!==item.id):[...current,item.id])}/></td><td className="px-4 py-3"><strong className="text-white">#{item.numero}</strong><span className="block text-xs text-slate-500">{formatDate(item.data)}</span></td><td className="px-4 py-3"><strong>{item.prefixo}</strong><span className="block max-w-44 truncate text-xs text-slate-500">{item.tipoEquipamento}</span></td><td className="px-4 py-3"><span className="block max-w-44 truncate">{item.operadorNome}</span><span className="text-xs text-slate-500">{item.matricula}</span></td><td className="px-4 py-3"><span className="block max-w-52 truncate">{item.obraNome}</span></td><td className="px-4 py-3 text-emerald-300">{formatNumber(getWorkedHours(item))} h</td><td className="px-4 py-3 text-amber-300">{formatNumber(getStoppedHours(item))} h</td><td className="px-4 py-3"><span className={deficiencies ? 'text-rose-300' : 'text-emerald-300'}>{deficiencies ? `${deficiencies} alerta(s)` : 'Sem alerta'}</span></td><td className="px-4 py-3"><span className={`inline-flex border px-2 py-1 text-xs font-bold ${statusTone[item.status]}`}>{item.status}</span></td><td className="px-4 py-3"><div className="flex gap-1"><button title="Visualizar" onClick={() => setPreviewRecord(item)} className="grid h-9 w-9 place-items-center text-slate-400 hover:bg-slate-800 hover:text-white"><Eye size={17} /></button><button title="Baixar PDF" onClick={() => downloadParteDiariaPdf(item, reneaLogoFull)} className="grid h-9 w-9 place-items-center text-slate-400 hover:bg-slate-800 hover:text-emerald-300"><FileDown size={17} /></button><button title="Editar" onClick={() => editRecord(item)} className="grid h-9 w-9 place-items-center text-slate-400 hover:bg-slate-800 hover:text-sky-300"><Edit3 size={17} /></button><button title="Duplicar" onClick={() => duplicateRecord(item)} className="grid h-9 w-9 place-items-center text-slate-400 hover:bg-slate-800 hover:text-amber-300"><CopyPlus size={17} /></button><button title="Excluir" onClick={() => onDelete(item.id)} className="grid h-9 w-9 place-items-center text-slate-400 hover:bg-slate-800 hover:text-rose-300"><Trash2 size={17} /></button></div></td></tr>; })}{!filtered.length && <tr><td colSpan={10} className="px-4 py-14 text-center text-slate-500">Nenhuma parte diária encontrada.</td></tr>}</tbody></table></div></section>}
