...
export default function FleetBulkActions({
  ...
  return (
    <aside className="sticky bottom-3 z-30 flex flex-col gap-2 rounded-lg border border-slate-300 bg-white p-3 text-slate-800 shadow-xl sm:flex-row sm:items-center">
      ...
      <select onChange={event => { if (event.target.value) onChangeStatus(event.target.value as FleetOperationalStatus); event.target.value = ''; }} defaultValue="" className="h-10 rounded-md border border-slate-600 bg-white px-3 text-xs font-black text-slate-800"><option value="" disabled>Alterar status...</option>{FLEET_STATUS_DEFINITIONS.filter(item=>item.key!=='unclassified').map(item=><option key={item.value}>{item.value}</option>)}</select>
      <button type="button" onClick={onExport} className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-600 px-3 text-xs font-black hover:bg-slate-50"><Download size={14}/>Exportar seleção</button>
