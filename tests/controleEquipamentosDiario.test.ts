...
const publicSnapshot = readFileSync(new URL('../api/_shared/cloud-snapshot.js', import.meta.url), 'utf8');
...
const dailyRecordForm = readFileSync(new URL('../src/components/fleet/DailyRecordForm.tsx', import.meta.url), 'utf8');
assert.doesNotMatch(dailyRecordForm, />Placa</);
assert.match(dailyRecordForm, /operationalFleet/);
assert.match(dailyRecordForm, /Pátio Aracaré/);
assert.match(dailyRecordForm, /Adicionar motorista manual/);
assert.doesNotMatch(dailyRecordForm, /Novo lançamento de basculante/);
